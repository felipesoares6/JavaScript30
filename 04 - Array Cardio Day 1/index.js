...
var years = inventors.reduce((total, inventor) => total + (inventor.passed - inventor.year), 0)
console.log(years);

// 5. Sort the inventors by years lived
var oldest = inventors.sort((a, b) => {
  const actualInventor = a.passed - a.year;
  const nextInventor = b.passed - b.year;
  return actualInventor > nextInventor ? -1 : 1
})
...
// //or [...categories.querySelectorAll('a')]
//
// const de = links
...
const peoplerSorted = people.sort((actual, next) => {
  const [actualA, nextA] = actual.split(', ');
  const [actualB, nextB] = next.split(', ');

  return actualA > actualB ? 1 : -1;
})

console.log(peoplerSorted);
